...
export default function ProductCategoriesModal({ isOpen, onClose, templates = [] }) {
  const { productCategories, addProductCategory, updateProductCategory, deleteProductCategory, isUsingLocalStorage } = useCategories()
  ...
  const initiateDelete = (cat) => {
    // Contar cuántas plantillas están usando esta categoría
    const count = templates.filter(t => t.category === cat.code).length
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="Gestionar Categorías de Producto" size="md">
        <div className="space-y-6">
          ...
          <form onSubmit={handleAdd} className="flex gap-2 items-end">
            <div className="flex-1">
              <Input
                label="Nueva Categoría de Producto"
                placeholder="Ej: Gorras, Camperas, Uniformes..."
                value={newCatName}
      ...
      <ConfirmDialog
        ...
        message={
          deleteCount > 0
            ? `¿Estás seguro de que deseas eliminar la categoría "${deleteTarget?.name}"? Hay ${deleteCount} plantilla(s) de producto que la utilizan actualmente. Nota: Al eliminarla, deberás reasignar las plantillas manualmente ya que su categoría quedará huérfana.`
